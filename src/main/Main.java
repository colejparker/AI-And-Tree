...
public class Main {
	...
	//Ask Cole how he wants to handle the addition of new slots?
	/*private static boolean AdditionalSlots(Node n) {
	...
	//Done normal constraint functions and most of hard constr, need to test them
	private static void Constr(Node n) {
		constrTotal = CourseMax(n) && Conflict(n) && Compatible(n) && Wanted(n);
		//deptConstrTotal = AdditionalSlots(n) && EveningCourses(n) && DiffSlots(n) && NoTuesday(n) && SpecialCourses(n)
		if((constrTotal && deptConstrTotal) == true) {
			n.nodeSolved();
		}
	}
